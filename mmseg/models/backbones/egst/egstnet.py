...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.conv_down(x)
        x = self.egconv1(x.transpose(1,2).reshape(B*T, self.embed_dim // 2, H // 2, W // 2))  # B*T, C, H, W
        x = x.reshape(B, T, self.embed_dim // 2, H // 2, W // 2).transpose(1, 2) # B, C, T, H, W

        x = self.conv_down2(x)
        x = self.egconv2(x.transpose(1,2).reshape(B*T//2, self.embed_dim, H // 4, W // 4))  # B, C*T, H, W
...
class EGSTNetLayer(torch.nn.Module):
    def __init__(
        self,
        in_chans,
        attn_type="local",
...
class EGSTNetBlock(torch.nn.Module):
    def __init__(
        self,
        in_chans,
        block_type="early",
        ...
        if block_type == "early":  # for early stages, i.e., stage1 and stage2 
            ...
            self.global_mixer = EGSTNetLayer(
                in_chans,
                "global",
                window_size=window_size,
                num_groups=num_groups,
...
class EGSTNetStage(torch.nn.Module):
