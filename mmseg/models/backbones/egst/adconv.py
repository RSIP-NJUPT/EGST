...
class ADConvWO45135(nn.Module):
    def __init__(self, dim):
        super(ADConvWO45135, self).__init__()
        self.dim = dim
        self.conv_shx = Conv2d_scharrx(dim)
        self.conv_shy = Conv2d_scharry(dim)
        # self.conv_sh45 = Conv2d_scharr45(dim)
...
class ADConvWOlaplacian(nn.Module):
    def __init__(self, dim):
        super(ADConvWOlaplacian, self).__init__()
        self.dim = dim
        self.conv_shx = Conv2d_scharrx(dim)
        self.conv_shy = Conv2d_scharry(dim)
        self.conv_sh45 = Conv2d_scharr45(dim)
...
class ADConvWO45135laplacian(nn.Module):
    def __init__(self, dim):
        super(ADConvWO45135laplacian, self).__init__()
        self.dim = dim
        self.conv_shx = Conv2d_scharrx(dim)
        self.conv_shy = Conv2d_scharry(dim)
        # self.conv_sh45 = Conv2d_scharr45(dim)
...
class ADConvBaseline(nn.Module):
    def __init__(self, dim):
        super(ADConvBaseline, self).__init__()
        self.dim = dim
        # self.conv_shx = Conv2d_scharrx(dim)
        # self.conv_shy = Conv2d_scharry(dim)
        # self.conv_sh45 = Conv2d_scharr45(dim)
...
class ADConvSobel(nn.Module):
    def __init__(self, dim):
        super(ADConvSobel, self).__init__()
        self.dim = dim
        self.conv_sbx = Conv2d_sobelx(dim)
        self.conv_sby = Conv2d_sobely(dim)
        self.conv_sb45 = Conv2d_sobel45(dim)
        self.conv_sb135 = Conv2d_sobel135(dim)
        self.conv_lpl = Conv2d_laplacian(dim)
        self.conv3x3 = nn.Conv2d(dim, dim, 3, padding=1, bias=True, groups=dim)

    def forward(self, x):
        w1, b1 = self.conv3x3.weight, self.conv3x3.bias
        w2, b2 = self.conv_sbx.get_weight()
        w3, b3 = self.conv_sby.get_weight()
        w4, b4 = self.conv_sb45.get_weight()
        w5, b5 = self.conv_sb135.get_weight()
        w6, b6 = self.conv_lpl.get_weight()
        w = w1 + w2 + w3 + w4 + w5 + w6
        b = b1 + b2 + b3 + b4 + b5 + b6
        out = nn.functional.conv2d(
...
class ADConvPrewitt(nn.Module):
    def __init__(self, dim):
        super(ADConvPrewitt, self).__init__()
        self.dim = dim
        self.conv_pwx = Conv2d_prewittx(dim)
        self.conv_pwy = Conv2d_prewitty(dim)
        self.conv_pw45 = Conv2d_prewitt45(dim)
        self.conv_pw135 = Conv2d_prewitt135(dim)
        self.conv_lpl = Conv2d_laplacian(dim)
        self.conv3x3 = nn.Conv2d(dim, dim, 3, padding=1, bias=True, groups=dim)

    def forward(self, x):
        w1, b1 = self.conv3x3.weight, self.conv3x3.bias
        w2, b2 = self.conv_pwx.get_weight()
        w3, b3 = self.conv_pwy.get_weight()
        w4, b4 = self.conv_pw45.get_weight()
        w5, b5 = self.conv_pw135.get_weight()
        w6, b6 = self.conv_lpl.get_weight()
        w = w1 + w2 + w3 + w4 + w5 + w6
        b = b1 + b2 + b3 + b4 + b5 + b6
        out = nn.functional.conv2d(
...
if __name__ == '__main__':
    ...
    # ADConvWO45135, ADConvWOlaplacian, ADConvWO45135laplacian, ADConvBaseline,
    net = ADConv(30).cuda()
    out = net(x)
    print(out.shape)
    pass
